...
#SBATCH --job-name=VIS_Sym_FSI_P1P0dc_stdres
...
ncpus=$SLURM_NTASKS
result_path=/scratch/user/zacharywegert/GridapTopOpt/results/Symmetric_FSI_3D_Burman_P1P0dc_MPI_superlu_0.01
...
srun -N $SLURM_JOB_NUM_NODES -n $SLURM_NTASKS --export=ALL julia --project -O3 \
  scripts/Embedded/Examples/FluidStructure/Burman-Stokes/Symmetric_FSI_3D_P1P0dc_MPI_superlu_stdres.jl \
  $ncpus \
  $result_path \
  $mesh_name \
  $I0 \
