...
function main(mesh_partition,distribute,el_size)
  ...
  vf = 0.4
  path = dirname(dirname(@__DIR__))*"/results/3d_thermal_compliance_ALM_SecondOrderStencil_Nx$(el_size[1])/"
  iter_mod = 10
  ...
  stencil = HamiltonJacobiEvolution(SecondOrderStencil(3,Float64),model,V_φ,tol,max_steps)
