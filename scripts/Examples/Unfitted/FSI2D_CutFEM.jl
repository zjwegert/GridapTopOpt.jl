...
"""
  ...
  - For R₂ above, j(d,s) is the displacement ghost penalty term over the ghost skeleton Γg
    ...
    isolated volumes marked by ψ_s. These are given by
        ...
        i(d,s) = ∫ ψ_s(d ⋅ s) dΩₛ.
"""
function main(ranks,model,geo_params;ls=LUSolver(),hilb_ls=LUSolver())
  # Output path
  ...
  ## Evolution Method
  evolve_nls = NewtonSolver(ls;maxiter=1,verbose=i_am_main(ranks))
  reinit_nls = NewtonSolver(ls;maxiter=20,rtol=1.e-14,verbose=i_am_main(ranks))
...
## Build refined model
function build_cells_to_refine(model,ranks,mesh_partition)
  # Create refinement map
  if isnothing(ranks) || isnothing(mesh_partition)
    ...
    error("Distributed refinement not implemented yet")
    cells_to_refine = map(local_views(model)) do model
      cells_to_refine_1 = convert(Vector{Bool},get_face_tag_index(get_face_labeling(model),"Omega_NonDesign",2))
      cells_to_refine_2 = convert(Vector{Bool},get_face_tag_index(get_face_labeling(model),"RefineBox",2))
      findall(cells_to_refine_1 .|| cells_to_refine_2)
    end
    cell_gids=get_cell_gids(model)
    cache=GridapDistributed.fetch_vector_ghost_values_cache(cells_to_refine,partition(cell_gids))
    GridapDistributed.fetch_vector_ghost_values!(cells_to_refine,cache)
  end
  return cells_to_refine
end

function build_model(nx,ny;L=2.0,H=0.5,x0=0.5,l=0.4,w=0.05,a=0.3,b=0.05,
    ranks=nothing,mesh_partition=nothing)
  ...
  if isnothing(ranks) || isnothing(mesh_partition)
    # ref_model = refine(ref_model; cells_to_refine=build_cells_to_refine(ref_model,ranks,mesh_partition))
    # ref_model = refine(ref_model; cells_to_refine=build_cells_to_refine(ref_model,ranks,mesh_partition))
    # ref_model = refine(ref_model; cells_to_refine=build_cells_to_refine(ref_model,ranks,mesh_partition))
  else
    @warn "Distributed refinement not implemented yet, skipping..."
  end
...
model, geo_params = build_model(200,50)
main(nothing,model,geo_params)

# with_debug() do distribute
#   mesh_partition = (2,2)
#   ranks = distribute(LinearIndices((prod(mesh_partition),)))
#   model, geo_params = build_model(4*20,20;b=1/20,w=1/20,ranks,mesh_partition)
