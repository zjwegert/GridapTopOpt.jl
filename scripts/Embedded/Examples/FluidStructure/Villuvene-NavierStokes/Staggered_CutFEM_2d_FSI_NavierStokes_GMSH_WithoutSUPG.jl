...
path = "./results/Staggered_CutFEM_2d_FSI_NavierStokes_GMSH_WithoutSUPG_gammag$γg_evo/"
mkpath(path)
...
γ_GPp(h)   = α_GPp*(μ/h)^-1*h^2 # (Eqn. 35, Villanueva and Maute, 2017)
...
r_Γ((u,p),(v,q),n) = -v⋅((σ_f ∘ (ε(u),p))⋅n) - u⋅((σ_f_β ∘ (ε(v),q))⋅n) + (γ_Nu ∘ hₕ)*u⋅v # (Eqn. 12, Villanueva and Maute, 2017)
r_ψ(p,q) = k_p * Ω.ψ_f*p*q # (Eqn. 15, Villanueva and Maute, 2017)
r_GP_μ(u,v) = mean(γ_GPμ ∘ hₕ)*jump(Ω.n_Γg ⋅ ∇(u)) ⋅ jump(Ω.n_Γg ⋅ ∇(v))
r_GP_p(p,q,n) = mean(γ_GPp ∘ hₕ)*jump(n ⋅ ∇(p)) * jump(n ⋅ ∇(q))
...
function res_fluid((),(u,p),(v,q),φ)
  ...
  return ∫(r_conv(u,v) + r_Ωf((u,p),(v,q)))Ω.dΩf +
    ∫(r_ψ(p,q))Ω.dΩf +
    ∫(r_Γ((u,p),(v,q),n_Γ))Ω.dΓ +
    ∫(r_GP_μ(u,v) + r_GP_p(p,q,Ω.n_Γg) + 0mean(φ))Ω.dΓg +
    ∫(r_GP_p(p,q,Ω.n_Λ_f) + 0mean(φ))Ω.dΛ_f
end
...
function jac_fluid_picard((),(u,p),(du,dp),(v,q),φ)
  ...
  return ∫(ρ*v ⋅ (conv∘(u,∇(du))) + r_Ωf((du,dp),(v,q)))Ω.dΩf +
    ∫(r_ψ(dp,q))Ω.dΩf +
    ∫(r_Γ((du,dp),(v,q),n_Γ))Ω.dΓ +
    ∫(r_GP_μ(du,v) + r_GP_p(dp,q,Ω.n_Γg) + 0mean(φ))Ω.dΓg +
    ∫(r_GP_p(dp,q,Ω.n_Λ_f) + 0mean(φ))Ω.dΛ_f
end
