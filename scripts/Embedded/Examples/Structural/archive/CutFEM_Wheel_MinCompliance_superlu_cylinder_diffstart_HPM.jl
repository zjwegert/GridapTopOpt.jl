...
function main(ranks)
  ...
  # Output path
  path = "./results/CutFEM_Wheel_MinCompliance_Neumann_gammag_$(γg_evo)_vf_$(vf)_superlu_cylinder_10step_agd_$(α_Gd)_diffstart/"
  ...
  _f2((x,y,z)) = -sqrt(x^2+y^2)+0.8
  _f3((x,y,z)) = sqrt(x^2+y^2)-0.21
  φh = interpolate(x->min(_f1(x,6,0.1),_f2(x),_f3(x)),V_φ)
