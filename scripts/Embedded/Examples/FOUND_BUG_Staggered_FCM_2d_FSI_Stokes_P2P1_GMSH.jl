...
path = "./results/Staggered_FCM_2d_FSI_Stokes_P2P1_GMSH/"#_NoDiff/"
mkpath(path)
...
a_fluid((),(u,p),(v,q),φ) =
  ...
  ∫( a_Ω(u,v)+b_Ω(u,q)+b_Ω(v,p)+(γ_Nu ∘ hₕ)*u⋅v )Ω.dΩs#NoDiff # Stabilization terms
...
function a_solid(((u,p),),d,s,φ)
  return ∫(a_s_Ω(s,d))Ω.dΩs + ∫(ϵ*a_s_Ω(s,d))Ω.dΩf#NoDiff
end
...
pcfs = PDEConstrainedFunctionals(J_pres,[Vol],state_map;analytic_dC=[dVol])
...
writevtk(Ω_act,path*"Omega_act",
      ...
      "λ1" => FEFunction(last(state_map.adj_caches).test[1],_K\_l),
      "λ2" => FEFunction(last(state_map.adj_caches).test,state_map.adj_caches[1])[2],
      "λ3" => FEFunction(last(state_map.adj_caches).test,state_map.adj_caches[1])[3],
      "uh"=>uh,"ph"=>ph,"dh"=>dh])
error()
