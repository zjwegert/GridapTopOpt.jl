...
function main(ranks)
  ...
  i_am_main(ranks) && println(" --- Load model")
  model = GmshDiscreteModel(ranks,(@__DIR__)*"/mesh_low_res_3d.msh")
  model = UnstructuredDiscreteModel(model)
  # map(test_mesh,local_views(model))
  writevtk(model,path*"model")

  i_am_main(ranks) && println(" --- Get element sizes")
  Ω_act = Triangulation(model)
  hₕ = get_element_diameter_field(model)
  hmin = minimum(get_element_diameters(model))

  # Cut the background model
  i_am_main(ranks) && println(" --- Create LSF")
  ...
  Ω = EmbeddedCollection(model,φh) do cutgeo,cutgeo_facets,_φh
    i_am_main(ranks) && println("     --- Build EmbeddedCollection")
    ...
    i_am_main(ranks) && println("     --- Compute masks")
  ...
  # Setup spaces
  i_am_main(ranks) && println(" --- Setup spaces")
  ...
  ## Evolution Method
  i_am_main(ranks) && println(" --- Setup Evo method")
  ...
  i_am_main(ranks) && println(" --- Reinit non-designable LSF")
  reinit!(ls_evo,φh_nondesign)

  ## Hilbertian extension-regularisation problems
  i_am_main(ranks) && println(" --- Setup Hilbertian extension-regularisation problems")
  _α(hₕ) = (α_coeff*hₕ)^2
