using Test

include("VelocityExtensionTests/runtests.jl")
include("UtilityTests/runtests.jl")
include("StateMapTests/runtests.jl")
include("EmbeddedTests/runtests.jl")
include("EvolverTests/runtests.jl")
include("ReinitialiserTests/runtests.jl")
include("GridapTopOptTests/runtests.jl")
include("GridapTopOptTests_unfitted/runtests.jl")
include("PolytopalCuttersTests/runtests.jl")