...
partition = (n,n)
domain = (0,1,0,1)
_bgmodel = CartesianDiscreteModel(domain,partition;isperiodic=(true,true))
update_labels!(1,_bgmodel,x->iszero(x[1]) || iszero(x[2]),"outer_boundary")
ref_model = refine(UnstructuredDiscreteModel(_bgmodel), refinement_method = "simplexify")
bgmodel = get_model(ref_model)

reffe = ReferenceFE(lagrangian,Float64,1)
V_φ = TestFESpace(ref_model,reffe)
φh = interpolate(x->(x[1]-0.5)^2+(x[2]-0.5)^2-0.55^2,V_φ)
geom = DiscreteGeometryFromFEFunction(φh,ref_model)

dp = 1
const h = dp/n

cutgeo = cut(PolytopalLevelSetCutter(), get_model(ref_model),geom)
