...
R = 0.7
n = 61 # TODO: This will fail if n = 60 because we currently don't allow the value of the ls to be zero at vertices
domain = (-1,1,-1,1)
partition = (n,n)
...
φh1 = interpolate(x->(x[1]-0.5)^2+(x[2]-0.1)^2-(0.4R)^2,V_φ)
φh4 = interpolate(x->(x[1]-0.2)^2+(x[2])^2-R^2,V_φ)
φh5 = interpolate(x->(x[1]+0.3)^2+(x[2])^2-(0.8R)^2,V_φ)
geo1 = DiscreteGeometryFromFEFunction(φh1,bgmodel)
geo4 = DiscreteGeometryFromFEFunction(φh4,bgmodel)
geo5 = DiscreteGeometryFromFEFunction(φh5,bgmodel)
