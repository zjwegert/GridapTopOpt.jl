...
path="./results/Thesis_example_heat_sink/"
rm(path,force=true,recursive=true)
mkpath(path)
n = 50
order = 1
γ = 0.1
max_steps = 10 #floor(Int,order*n/10*3)
vf = 0.3
α_coeff = 2
iter_mod = 1
...
ref_model = refine(ref_model)#, refinement_method = "barycentric")
ref_model = refine(ref_model)#, refinement_method = "barycentric")
model = ref_model.model
h = minimum(get_element_diameters(model))
hₕ = get_element_diameter_field(model)
f_Γ_D(x) = (x[1]-0.5)^2 + (x[2]-0.5)^2 <= 0.05^2 #(0.45 - eps() <= x[1] <= 0.55 + eps()) && (0.45 - eps() <= x[2] <= 0.55 + eps())
...
# f3 = (x,y) -> max(abs(x-0.5),abs(y-0.5)) - 0.06
