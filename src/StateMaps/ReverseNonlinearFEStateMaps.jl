...
(of_at_φ::ParameterisedFunction)(x) = of_at_φ.of( x, of_at_φ.φ)

Gridap.get_free_dof_values(pf::ParameterisedFunction) = pf.φ

struct Scaled{A,F}
    a::A
    f::F
end
(s::Scaled)(x...) = s.a * s.f(x...)

Base.:*(a::Number, f::ParameterisedFunction) = Scaled(a, f)

"""
    struct ReverseParameterisedStateMap{A,B,C,D,E} <: AbstractFEStateMap
...
- `jacs`: a tuple of `Function`s defining the Jacobian of the residual and
  the Jacobian for the adjoint. This allows the user to specify a different Jacobian
  for the forward problem/adjoint problem (e.g., Picard iterations).
- `spaces`: `Tuple` of finite element spaces and the parameterised function 
- `assems`: `Tuple` of assemblers
- `cache`: A FEStateMapCache
"""
struct ReverseParameterisedStateMap{A,B,C,D,E} <: AbstractFEStateMap
  ...
  @doc """
      ReverseParameterisedStateMap(
        res::Function,jac::Function,U,V,of;
        assem_U = SparseMatrixAssembler(U,V),
        assem_adjoint = SparseMatrixAssembler(V,U),
        # assem_deriv = SparseMatrixAssembler(V_φ,V_φ),
        nls::NonlinearSolver = NewtonSolver(LUSolver();maxiter=50,rtol=1.e-8,verbose=true),
  ...
  Create an instance of `ReverseParameterisedStateMap` given the residual `res` as a `Function` type,
  trial and test spaces `U` and `V`, the function `of` that is parameterised by φ,
  and the measures as additional arguments. The space `V_diff` is the FE space is used to assist with AD. It will be removed in the future.
  ...
  for the forward problem (e.g., for picard iterations).
  """
  function ReverseParameterisedStateMap(
    res::Function,jac::Function,U,V,of;
    assem_U = SparseMatrixAssembler(U,V),
    assem_adjoint = SparseMatrixAssembler(V,U),
    #assem_deriv = SparseMatrixAssembler(V_φ,V_φ),
...
function ReverseParameterisedStateMap(res::Function,U,V,of;kwargs...)
  jac = (u,du,v,of_at_φ) -> Gridap.jacobian(u -> res(u,v,of_at_φ),u)
  ReverseParameterisedStateMap(res,jac,U,V,of;kwargs...)
end

# Caching
function build_cache!(state_map::ReverseParameterisedStateMap,of_at_φ::ParameterisedFunction)
  assem_U, assem_adjoint = state_map.assems
  U,V,of = state_map.spaces
  res = state_map.res
  jac, adjoint_jac = state_map.jacs
  cache = state_map.cache
  nls, adjoint_ls = cache.solvers[1], cache.solvers[2]

  # ## Pullback cache # we could implement this in the future if the cache needs to be larger 
  # dudφ_vec = [0.0,0.0] # change to a proper function like zero dof values
  # cache.plb_cache = (dudφ_vec,assem_deriv)

  ## Forward cache
  x = zero_free_values(U)
  _res(u,v) = res(u,v,of_at_φ)
  _jac(u,du,v) = jac(u,du,v,of_at_φ)
  op = get_algebraic_operator(FEOperator(_res,_jac,U,V,assem_U))
  nls_cache = GridapTopOpt.instantiate_caches(x,nls,op)
  cache.fwd_cache = (nls,nls_cache,x)

  ## Adjoint cache
  uhd = zero(U)
  _jac_adj(du,v) = adjoint_jac(uhd,du,v,of_at_φ)
  adjoint_K  = assemble_adjoint_matrix(_jac_adj,assem_adjoint,U,V)
  adjoint_x  = allocate_in_domain(adjoint_K); fill!(adjoint_x,zero(eltype(adjoint_x)))
  adjoint_ns = numerical_setup(symbolic_setup(adjoint_ls,adjoint_K),adjoint_K)
  cache.adj_cache = (adjoint_ns,adjoint_K,adjoint_x)
...
# Getters
function get_state(m::ReverseParameterisedStateMap)
  @assert is_cache_built(m.cache) """
    You must build the cache before using get_state. This can be achieved by either
    solving your problem with my_state_map(of_at_φ) or by running build_cache!(my_state_map,of_at_φ)
  """
  FEFunction(get_trial_space(m),m.cache.fwd_cache[3])
end
#get_plb_cache(m::ReverseParameterisedStateMap) = m.cache.plb_cache
get_spaces(m::ReverseParameterisedStateMap) = m.spaces
get_assemblers(m::ReverseParameterisedStateMap) = m.assems

function forward_solve!(φ_to_u::ReverseParameterisedStateMap,of_at_φ::ParameterisedFunction)
  U, V, of = φ_to_u.spaces
  assem_U = φ_to_u.assems.assem_U
  res=φ_to_u.res
  jac,_=φ_to_u.jacs
  if !is_cache_built(φ_to_u.cache)
    build_cache!(φ_to_u,of_at_φ)
  end
  nls, nls_cache, x = φ_to_u.cache.fwd_cache
...
function forward_solve!(φ_to_u::ReverseParameterisedStateMap,φ::AbstractVector) 
  of = φ_to_u.spaces[3]
  of_at_φ = ParameterisedFunction(of, φ)
  return forward_solve!(φ_to_u,of_at_φ)
end

function dRdφ(φ_to_u::ReverseParameterisedStateMap,uh,vh,of_at_φ::ParameterisedFunction)
  res = φ_to_u.res
  of = φ_to_u.spaces[3]
  function _res(φ)
    of_at_φ = ParameterisedFunction(of, φ)
    sum(res(uh,vh,of_at_φ))
  end                                                                    
  return ReverseDiff.gradient(_res,get_free_dof_values(of_at_φ))
end

function update_adjoint_caches!(φ_to_u::ReverseParameterisedStateMap,uh,of_at_φ::ParameterisedFunction)
  if !is_cache_built(φ_to_u.cache)
    build_cache!(φ_to_u,of_at_φ)
  end
  assem_adjoint = φ_to_u.assems.assem_adjoint
  ...
  _,adjoint_jac=φ_to_u.jacs
  U, V, of = φ_to_u.spaces
  jac(du,v) =  adjoint_jac(uh,du,v,of_at_φ)
  assemble_adjoint_matrix!(jac,adjoint_K,assem_adjoint,U,V)
  numerical_setup!(adjoint_ns,adjoint_K)
  return φ_to_u.cache.adj_cache
end

function adjoint_solve!(φ_to_u::ReverseParameterisedStateMap,du::AbstractVector)
  adjoint_ns, _, adjoint_x = φ_to_u.cache.adj_cache
  solve!(adjoint_x,adjoint_ns,du)
  return adjoint_x
end

function pullback(φ_to_u::ReverseParameterisedStateMap,uh,of_at_φ,du;updated=false)
  #dudφ_vec = get_plb_cache(φ_to_u)

  ## Adjoint Solve
  if !updated
    update_adjoint_caches!(φ_to_u,uh,of_at_φ)
  end
  λ  = adjoint_solve!(φ_to_u,du)
  λh = FEFunction(get_test_space(φ_to_u),λ)

  ## Compute grad
  dudφ_vec = dRdφ(φ_to_u,uh,λh,of_at_φ)
  rmul!(dudφ_vec, -1)

  return (NoTangent(),dudφ_vec)
end

function ChainRulesCore.rrule(φ_to_u::ReverseParameterisedStateMap,of_at_φ::ParameterisedFunction)
  u  = forward_solve!(φ_to_u,of_at_φ)
  uh = FEFunction(get_trial_space(φ_to_u),u)
  update_adjoint_caches!(φ_to_u,uh,of_at_φ)
  return u, du -> pullback(φ_to_u,uh,of_at_φ,du;updated=true)
end

function ChainRulesCore.rrule(φ_to_u::ReverseParameterisedStateMap,φ::AbstractVector)
  of = φ_to_u.spaces[3]
  of_at_φ = ParameterisedFunction(of, φ)
  return ChainRulesCore.rrule(φ_to_u,of_at_φ)
end