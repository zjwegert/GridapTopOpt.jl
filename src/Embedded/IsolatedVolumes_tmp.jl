...
function get_isolated_volumes_mask(
  ...
  cell_to_color, color_to_group, cell_to_nbors = tag_disconnected_volumes(model,bgcell_to_inoutcut;groups)
  color_to_isolated = find_isolated_volumes(model,dirichlet_tags,cell_to_color,color_to_group)

  alt_groups = ((CUT,first(groups)),first(groups)*IN)
  ALT_cell_to_color, ALT_color_to_group, _ = tag_disconnected_volumes(model,bgcell_to_inoutcut;groups=alt_groups)
  ALT_color_to_isolated = find_isolated_volumes(model,dirichlet_tags,ALT_cell_to_color,ALT_color_to_group)
  ...
  data2 = map(c -> ALT_color_to_isolated[c] && isone(ALT_color_to_group[c]), ALT_cell_to_color)

  data = data .| data2

  remove_cuts && add_cuts!(data,bgcell_to_inoutcut,cell_to_nbors)
  ...
  for neighbors in cell_to_nbors[data]
    for j in neighbors
      if !data[j] && cell_to_inoutcut[j] == CUT
        data[j] = true
        for k in cell_to_nbors[j]
          if !data[k] && cell_to_inoutcut[k] == OUT
            data[k] = true
          end
        end
      end
...
function tag_disconnected_volumes(
  ...
  cell_to_lcolor, lcolor_to_group, lcell_to_nbors = map(local_views(model),cell_to_state) do model, cell_to_state
