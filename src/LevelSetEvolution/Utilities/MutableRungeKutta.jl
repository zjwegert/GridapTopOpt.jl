import Gridap.ODEs: allocate_odecache,ode_march!,allocate_odecache,
    ...
    allocate_odeopcache,ODEOperatorType,is_form_constant,allocate_jacobian,allocate_residual,
    NumericalSetup,update_odeopcache!,LinearStageOperator,_setindex_all!,axpy!
...
function allocate_odecache(
  odeslvr::MutableDIMRungeKutta, odeop::ODEOperator,
...
###############
function allocate_odecache(
  odeslvr::MutableDIMRungeKutta, odeop::ODEOperator{<:AbstractLinearODE},
  t0::Real, us0::NTuple{1,AbstractVector}
)
  u0 = us0[1]
  us0N = (u0, u0)
...
function MutableRungeKutta(
  ...
  elseif type == DiagonallyImplicitTableau
    MutableDIMRungeKutta(sysslvr_nl, sysslvr_l, dt, tableau)
...
MutableRungeKutta(sysslvr_nl, sysslvr_l, dt, ButcherTableau(name))
end

function MutableRungeKutta(sysslvr_nl::NonlinearSolver, dt::Real, tableau)
  MutableRungeKutta(sysslvr_nl, sysslvr_nl, dt, name)
end
